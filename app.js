...
function setLoading(v)      { loadingBox.classList.toggle('hidden', !v); }
function setEmpty(v)        { emptyState.classList.toggle('hidden', !v); }
function setTableVisible(v) { tableWrapper.classList.toggle('hidden', !v); }
...
function buildRow(item) {
  ...
  tr.innerHTML = `
    ...
    <td>
      <a class="course-link" href="${escapeHtml(item.course_url)}" target="_blank">
        Open course
      </a>
    </td>
    <td>
...
async function loadTable() {
  try {
    ...
    data.items.forEach(item => fragment.appendChild(buildRow(item)));
...
// ─── Bootstrap ───────────────────────────────────────────────────────────────
// Czekamy na sygnał z initApp() w index.html, żeby mieć pewność że sesja
// jest już ustawiona zanim zaczniemy robić requesty do API.

refreshBtn.addEventListener('click', loadTable);

if (window.APP_READY) {
  // initApp() skończyło się zanim app.js się załadował
  loadTable();
} else {
  // Czekamy na event z initApp()
  document.addEventListener('approval-user-ready', loadTable);
}
